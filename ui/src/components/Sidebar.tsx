...
export function Sidebar({ screen, onNavigate, overview }: SidebarProps) {
  ...
  return (
    <aside className="flex w-60 flex-col border-r border-taupe bg-mist-gray">
      {/* Brand */}
      <div className="flex h-16 items-center gap-3 border-b border-taupe px-5">
        ...
        <div>
          <div className="text-sm font-bold tracking-wider text-graphite">
            NEXARA
          </div>
          <div className="text-[10px] uppercase tracking-widest text-stone/60">
            主脑控制台
          </div>
        </div>
      </div>
      ...
      <div className="border-b border-taupe px-5 py-4">
        ...
        <div className="mt-1 flex items-center justify-between">
          ...
          <span
            className={cn(
              "text-sm font-bold",
              pendingApprovals > 0 ? "text-amber" : "text-stone/40"
            )}
          >
            {pendingApprovals}
          </span>
      ...
      {/* Mission workspace shortcut */}
      {overview?.missions?.length ? (
        <div className="border-t border-taupe px-3 py-4">
          <div className="mb-2 text-[10px] uppercase tracking-widest text-stone/50">
            最近任务
          </div>
          {overview.missions.slice(-3).reverse().map((m) => (
            <div
              key={m.mission_id}
              className="block w-full truncate rounded-md px-2 py-1 text-left text-xs text-stone/70"
