...
echo "UPGRADE_SHA=$(shasum -a 256 com.nexara.runtime.plist | awk '{print $1}')"
launchctl kickstart -k gui/$(id -u)/com.nexara.runtime 2>/dev/null
sleep 8
...
curl -s -X POST -H "Content-Type: application/json" -d '{"title":"upgrade-v2"}' http://127.0.0.1:8765/api/conversations -o /tmp/uc.json
...
echo "ROLLBACK_SHA=$(shasum -a 256 com.nexara.runtime.plist | awk '{print $1}')"
launchctl kickstart -k gui/$(id -u)/com.nexara.runtime 2>/dev/null
sleep 8
