...
完成一个局部任务后不得默认停止。
Skill 安装或 Skill 文件创建完成只代表局部 Mission 完成，不代表 NEXARA Program 完成。
只有遇到本技能定义的停止条件才可以退出 Program Loop。
## 8. 产品主线保护
在选择下一任务时，优先级为：
...
局部 Mission PASS 后，必须读取现有 Program State 并选择下一 Mission。
禁止仅因 Skill 文件创建完成而输出 `next_mission: N/A` 或 `program_loop: STOPPED`。
## 21. 核心执行原则
主线优先，不跨项目。
