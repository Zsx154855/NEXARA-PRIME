...
from .models import DriftFinding, DriftType, ProductTwinCheckpoint, TwinSnapshot


class ProductTwinEngine:
    """Capture expected and observed product state and detect deterministic drift."""
    ...
    def _diff(
        ...
        findings: list[tuple[str, Any, Any]] = []

        if isinstance(expected, dict) and isinstance(observed, dict):
            for key in sorted(set(expected) | set(observed)):
                child_path = f"{path}.{key}"
                if key not in expected:
                    findings.append((child_path, None, observed[key]))
                elif key not in observed:
                    findings.append((child_path, expected[key], None))
                else:
        ...
        if isinstance(expected, list) and isinstance(observed, list):
            if expected != observed:
                findings.append((path, expected, observed))
            return findings
